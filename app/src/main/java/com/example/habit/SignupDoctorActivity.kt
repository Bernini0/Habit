...
class SignupDoctorActivity : BaseActivity(), View.OnClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mEdtDesignation = findViewById(R.id.signup_doctor_designation_field)
        mEdtName = findViewById(R.id.signup_doctor_name_field)
        mEdtBio = findViewById(R.id.signup_doctor_bio_field)

        mLayoutEmail = findViewById(R.id.layout_signup_email)
        mLayoutPassword = findViewById(R.id.layout_signup_password)
        mLayoutDesignation = findViewById(R.id.layout_designation)
        mLayoutDesignation = findViewById(R.id.layout_name)
        mLayoutBio = findViewById(R.id.layout_bio)
        ...
        mAuthListener = FirebaseAuth.AuthStateListener { firebaseAuth ->
            ...
            if (user != null) {
                ...
                val usr = User(user.uid, mEdtName!!.text.toString(), mEdtEmail!!.text.toString(), mEdtDesignation!!.text.toString(), "doctor", mEdtBio!!.text.toString(), 0.0, mutableListOf())
    ...
    override fun onClick(v: View?) {
        when(v?.id) {
            ...
            R.id.go_to_login_text -> {
                val intent = Intent(this, LoginDcotorActivity::class.java)
    ...
    private fun validateForm(): Boolean {
        return if (TextUtils.isEmpty(mEdtEmail?.getText().toString())) {
            ...
            false
        } else if (TextUtils.isEmpty(mEdtDesignation?.getText().toString())) {
           mLayoutDesignation?.error = "Required"
            false
        } else if (TextUtils.isEmpty(mEdtName?.getText().toString())) {
            mLayoutName?.error = "Required"
            false
        } else {
